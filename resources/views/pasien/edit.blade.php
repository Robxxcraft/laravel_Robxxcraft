    ...
    <div class="card-body">
        ...
        <form action="/pasien/tambah" method="POST" class="p-3">
            ...
            <div class="pb-3 position-relative">
                ...
                <input type="text" name="nama_pasien" id="nama_pasien" class="form-control mb-1 {{ $errors->has('nama_pasien') ? 'is-invalid' : ''}}" placeholder="Masukan Nama Rumah Sakit disini..." aria-label="nama_pasien" autocomplete="false" value="{{$pasien->nama_pasien}}">
                @error('nama_pasien')
            ...
            <div class="pb-3 position-relative">
                ...
                <textarea name="alamat" id="alamat" class="form-control mb-1 {{ $errors->has('alamat') ? 'is-invalid' : ''}}" placeholder="Masukan alamat disini..." aria-label="alamat">{{$pasien->alamat}}</textarea>
                @error('alamat')
                    <div class="invalid-feedback position-absolute bottom-0 ps-2">{{$message}}</div>
                @enderror
            </div>
            <div class="pb-3 position-relative">
                ...
                <input type="text" name="no_telepon" id="telepon" class="form-control mb-1 {{ $errors->has('no_telepon') ? 'is-invalid' : ''}}" placeholder="Masukan telepon disini..." aria-label="telepon" value="{{$pasien->no_telepon}}">
                @error('no_telepon')
                    <div class="invalid-feedback position-absolute bottom-0 ps-2">{{$message}}</div>
                @enderror
            </div>
            <div class="pb-3 position-relative">
                ...
                <select name="id_rumah_sakit" id="nama_rumah_sakit" class="form-control {{$errors->has('id_rumah_sakit') ? 'is-invalid' : ''}}">
                    ...
                    @foreach ($RumahSakit as $rs)
                        @if ($rs->id == $pasien->rumah_sakit->id)
                        <option selected value="{{$rs->id}}">{{$rs->nama_rumah_sakit}}</option>
                        @else
